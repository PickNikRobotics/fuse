...
TEST(TestSuite, mergeFullPositionAndFullOrientationIndices)
{
  ...
  EXPECT_THAT(position_indices,
              testing::ElementsAreArray(merged_indices.begin(), merged_indices.begin() + position_indices.size()));
...
TEST(TestSuite, populatePartialMeasurements)
{
  ...
  std::replace_if(
      ...
      [&merged_indices, &pose_mean_partial](double const& value) {
        return std::find(merged_indices.begin(), merged_indices.end(), &value - pose_mean_partial.data()) ==
               merged_indices.end();
      },
      0.0);
  ...
  fm_common::populatePartialMeasurement(pose_covariance, merged_indices, pose_covariance_partial);
